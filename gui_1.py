...
from socket import *def handleCalc():   
 IP = '0.0.0.0'    
 PORT = 50000    
 BUFLEN = 512   
 listenSocket = socket(AF_INET,SOCK_STREAM)    listenSocket.bind((IP,PORT))   
 ...
 dataSocket.send(f'服务端收到了信息{info}'.encode())        dataSocket.close()    
 listenSocket.close()app = QApplication([])
 window = QMainWindow()window.resize(500, 400)
 window.move(300, 310)
